...
{#if withDifficulty}
  <Tabs.Root value="advanced" class={cn('flex w-full flex-col gap-1', className)}>
    ...
    <Tabs.Content value="advanced" class="items-top flex gap-2">
      <FormItem {label} class="w-full">
        {#if textarea}
          <Textarea bind:value={text} id="filter" class="min-h-8" />
        {:else}
          <Input bind:value={text} id="name" class="h-8" />
        {/if}
      </FormItem>
    </Tabs.Content>
  </Tabs.Root>
